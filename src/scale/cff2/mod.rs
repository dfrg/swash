...
use read_fonts::types::{F2Dot14, GlyphId, Pen};
...
impl SubfontCache {
    ...
    pub fn scale(
        ...
        let subfont_index = scaler.subfont_index(gid);
        ...
        let (subfont, coords) = if found {
            ...
            if entry_index == self.entries.len() {
                ...
                let subfont = scaler.subfont(subfont_index, size, &coords).ok()?;
                ...
                entry.subfont = scaler.subfont(subfont_index, size, &entry.coords).ok()?;
                entry.id = id;
                (&entry.subfont, &entry.coords)
            }
        };
        scaler
            .outline(subfont, gid, coords, hint, &mut OutlineBuilder(outline))
            .ok()?;
        Some(())
    }
    ...
    fn find_entry(&self, id: u64, index: u32, coords: &[i16], size: f32) -> (bool, usize) {
        ...
        for (i, entry) in self.entries.iter().enumerate() {
            if entry.id == id
                && entry.subfont.index() == index
                && entry.subfont.size() == size
                && coords
                    .iter()
